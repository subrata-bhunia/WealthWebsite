...
export default function MediaPage() {
  ...
  const youtubeMedia = mediaItems.filter(
    (item) => item.mediaType === 'youtube' || (item.mediaType === 'both' && item.youtubeUrl)
  );
  
  const fileMedia = mediaItems.filter(
    (item) => item.mediaType === 'file' || (item.mediaType === 'both' && item.fileUrl)
  );
    ...
    >
      <div className="container mx-auto px-4 py-12">
        {isLoading ? (
          ...
          <div className="text-center text-red-500">Error loading media: {(error as Error).message}</div>
        ) : mediaItems.length === 0 ? (
                ...
                {mediaItems.map((item) => (
                  <Card key={item.id} className="h-full flex flex-col">
                    ...
                    <CardContent className="flex-grow">
                      {item.youtubeUrl && (item.mediaType === 'youtube' || item.mediaType === 'both') && (
                        <div className="mb-4">
                      ...
                      {item.fileUrl && (item.mediaType === 'file' || item.mediaType === 'both') && (
                        <div>
                          <div className="flex items-center gap-2 font-semibold text-sm mb-2">
